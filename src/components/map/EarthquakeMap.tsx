...
function getZoomFactor({ zoom, zoomOffset = 0 }: { zoom: number; zoomOffset?: number }) {
...
export default function EarthquakeMap() {
  ...
  const toggleSyncCharts = () => {
    ...
    if (!next && clientRef.current) {
      brush.update({ source: clientRef.current, value: null, predicate: null as any });
    }
  };
  ...
  return (
    <div className="flex w-screen h-screen overflow-hidden bg-[#1b1a18]">
      <div className="flex-1 relative">
        ...
        >
          <Map mapboxAccessToken={MAPBOX_TOKEN} mapStyle="mapbox://styles/mapbox/dark-v11" projection="mercator" />
        </DeckGL>
        ...
        {!dbReady && (
          <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black/40 backdrop-blur-sm z-40 text-white">
      ...
      <div className="w-[420px] h-full bg-[#f3efe7] border-[#2a2825] text-slate-100 flex flex-col">
        {dbReady && <EarthquakeCharts />}
        {!dbReady && (
          <div className="flex-1 flex items-center justify-center p-4">
            <Loader2 className="w-8 h-8 animate-spin mr-2 text-slate-800" />
          </div>
        )}
      </div>
    </div>
