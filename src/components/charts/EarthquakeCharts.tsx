...
import { createMagPlot, createDepthPlot, createTimePlot } from "./plotDefinitions";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export default React.memo(function EarthquakeCharts() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f3efe7] text-slate-800">
      <ScrollArea className="flex-1">
        <div className="p-2">
          ...
          >
            <AccordionItem value="magnitude" className="rounded-sm px-2 border border-[#d8d2c7] bg-white/60">
              <AccordionTrigger className="hover:no-underline py-2">
                <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                  Distribution by Magnitude
                </div>
              </AccordionTrigger>
              <AccordionContent className="pb-4 flex overflow-hidden">
                {!ready ? <Skeleton h={180} /> : <VgPlot plot={magPlot} />}
              </AccordionContent>
            </AccordionItem>

            <AccordionItem value="timeline" className="rounded-sm px-2 border border-[#d8d2c7] bg-white/60">
              <AccordionTrigger className="hover:no-underline py-2">
                <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                  Temporal Frequency
                </div>
            ...
            <AccordionItem value="depth" className="rounded-sm px-2 border border-[#d8d2c7] bg-white/60">
              <AccordionTrigger className="hover:no-underline py-2">
                <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                  Depth vs Magnitude
                </div>
